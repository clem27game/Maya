...
void handle_discord_connect(char *line) {
    ...
    *comma = '\0';
    ...
    if (strlen(token) < 50) {
        maya_error("Token Discord invalide (trop court - minimum 50 caractères)", 0);
        return;
    }

    // Vérifier le format du token Discord
    if (strncmp(token, "Bot ", 4) == 0) {
        token += 4; // Enlever le prefix "Bot " si présent
    }
    ...
    // Initialiser les intents par défaut requis
    discord_bot.intents_enabled[0] = 1; // GUILD_MESSAGES
    discord_bot.intents_enabled[1] = 1; // DIRECT_MESSAGES  
    discord_bot.intents_enabled[2] = 1; // MESSAGE_CONTENT

    printf("🤖 DISCORD.MY - CONNEXION BOT 🤖\n");
    printf("Token configuré: %.*s...\n", 20, token);
    printf("Prefix: %s\n", prefix);
    printf("🔗 Test de connexion à l'API Discord...\n");
    
    // Tester la connexion avec /users/@me
    if (!real_discord_api_call("/users/@me", "GET", "", token)) {
        printf("❌ Échec de la connexion à Discord\n");
        printf("💡 Vérifications à faire:\n");
        printf("   1. Token valide et actif\n");
        printf("   2. Bot créé dans Discord Developer Portal\n");
        printf("   3. Connexion internet active\n");
        discord_bot.bot_active = 0;
        return;
    }
    
    // Tester la gateway Discord
    printf("🌐 Test de la gateway Discord...\n");
    if (!real_discord_api_call("/gateway/bot", "GET", "", token)) {
        printf("⚠️ Gateway inaccessible mais bot connecté\n");
    }
    
    printf("✅ Bot Discord connecté avec succès!\n");
    printf("🎯 Prêt à recevoir des commandes avec le prefix: %s\n", prefix);
    printf("📋 Intents activés: MESSAGE_CONTENT, GUILD_MESSAGES, DM_MESSAGES\n");
}
...
int real_discord_api_call(const char *endpoint, const char *method, const char *data, const char *token) {
    ...
    if (response_code >= 200 && response_code < 300) {
        ...
        if (response_code == 401) {
            printf("🔑 Token invalide ou manquant\n");
        } else if (response_code == 403) {
            printf("🚫 Permissions insuffisantes\n");
        } else if (response_code == 429) {
            printf("⏰ Rate limit atteint, ralentissez les requêtes\n");
        } else if (response_code >= 500) {
            printf("🔧 Erreur serveur Discord, réessayez plus tard\n");
        }
    }
...
int send_discord_message(const char *channel_id, const char *content) {
    if (!channel_id || !content || strlen(channel_id) == 0 || strlen(content) == 0) {
        printf("❌ Channel ID ou contenu manquant\n");
        return 0;
    }
    
    if (strlen(content) > 2000) {
        printf("❌ Message trop long (max 2000 caractères)\n");
        return 0;
    }
    ...
    printf("📤 Envoi message vers channel %s: %.50s%s\n", 
           channel_id, content, strlen(content) > 50 ? "..." : "");
...
int send_discord_embed(const char *channel_id, const char *title, const char *description, 
    ...
    if (title && strlen(title) > 0) {
        if (strlen(title) > 256) {
            printf("⚠️ Titre tronqué (max 256 caractères)\n");
            char truncated_title[257];
            strncpy(truncated_title, title, 256);
            truncated_title[256] = '\0';
            json_object *title_obj = json_object_new_string(truncated_title);
    ...
    // Description (max 4096 caractères)
    if (description && strlen(description) > 0) {
        if (strlen(description) > 4096) {
            printf("⚠️ Description tronquée (max 4096 caractères)\n");
            char truncated_desc[4097];
            strncpy(truncated_desc, description, 4096);
            truncated_desc[4096] = '\0';
            json_object *desc_obj = json_object_new_string(truncated_desc);
    ...
    if (color && strlen(color) > 0) {
        int color_int = 0;
        if (color[0] == '#') {
            color_int = (int)strtol(color + 1, NULL, 16);
        } else {
            color_int = (int)strtol(color, NULL, 16);
        }
        json_object *color_obj = json_object_new_int(color_int);
        json_object_object_add(embed_obj, "color", color_obj);
    }
    
    // Footer (max 2048 caractères)
    if (footer && strlen(footer) > 0) {
        json_object *footer_obj = json_object_new_object();
        if (strlen(footer) > 2048) {
            printf("⚠️ Footer tronqué (max 2048 caractères)\n");
            char truncated_footer[2049];
            strncpy(truncated_footer, footer, 2048);
            truncated_footer[2048] = '\0';
            json_object *footer_text = json_object_new_string(truncated_footer);
    ...
    json_object_object_add(embed_obj, "timestamp", timestamp_obj);
    ...
    printf("📤 Envoi embed vers channel %s\n", channel_id);
    if (title) printf("   📋 Titre: %.50s%s\n", title, strlen(title) > 50 ? "..." : "");
    
    int success = real_discord_api_call(endpoint, "POST", json_string, discord_bot.token);
    
    if (success) {
        printf("✅ Embed envoyé avec succès\n");
    } else {
        printf("❌ Échec de l'envoi de l'embed\n");
    }
...
// Variables pour le serveur HTTP
static int server_socket = -1;
static int server_running = 0;

// Fonction pour démarrer un serveur HTTP simple pour les interactions
int start_http_server(int port) {
    ...
    if (bind(server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        printf("❌ Erreur bind socket sur port %d\n", port);
        close(server_socket);
        return 0;
    }
...
void* discord_bot_thread(void* arg) {
    ...
    // Configurer l'activité si définie
    if (strlen(discord_bot.activity_name) > 0) {
        char activity_json[512];
        snprintf(activity_json, sizeof(activity_json),
                "{\"activities\":[{\"name\":\"%s\",\"type\":%s}]}",
                discord_bot.activity_name,
                strcmp(discord_bot.activity_type, "playing") == 0 ? "0" :
                strcmp(discord_bot.activity_type, "listening") == 0 ? "2" :
                strcmp(discord_bot.activity_type, "watching") == 0 ? "3" : "0");
        real_discord_api_call("/users/@me/settings", "PATCH", activity_json, discord_bot.token);
    }
    
    // Démarrer serveur HTTP pour les interactions
    if (!start_http_server(5000)) {
        printf("⚠️ Serveur HTTP non démarré, fonctionnement en mode API seulement\n");
    }
    
    printf("🤖 Bot Discord actif!\n");
    printf("📋 Commandes configurées: %d\n", discord_command_count);
    printf("🎯 Prefix: %s\n", discord_bot.prefix);
    printf("⚠️  Appuyez sur Ctrl+C pour arrêter le bot\n");
    printf("💡 Pour tester, invitez le bot sur un serveur et utilisez les commandes!\n");
    
    // Boucle principale
    while (websocket_connected) {
        // Accepter les connexions HTTP si le serveur est actif
        if (server_running && server_socket >= 0) {
            ...
            if (activity > 0 && FD_ISSET(server_socket, &read_fds)) {
                ...
                if (client_socket >= 0) {
                    // Répondre avec un simple status
                    char response[] = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n{\"status\":\"Maya Discord Bot Active\",\"commands\":%d}";
                    char full_response[512];
                    snprintf(full_response, sizeof(full_response), response, discord_command_count);
                    send(client_socket, full_response, strlen(full_response), 0);
                    close(client_socket);
                    
                    printf("📡 Requête HTTP reçue de %s\n", inet_ntoa(client_addr.sin_addr));
                }
            }
        }
        
        // Simulation de ping pour maintenir la connexion
        static int ping_counter = 0;
        if (++ping_counter >= 30) { // Toutes les 30 secondes
            printf("💓 Bot actif - %d commandes prêtes\n", discord_command_count);
            ping_counter = 0;
        }
